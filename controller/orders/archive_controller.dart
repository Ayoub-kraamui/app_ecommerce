...
import 'package:app_ecommerce/data/datasource/remote/orders/archive.dart';
...
class OrdersArchiveController extends GetxController {
  ...
  submitRating(int orderid, double rating, String coment) async {
    statusRequest = StatusRequest.loading;
    update();
    var response = await ordersArchiveData.ratingData(
        orderid.toString(), rating.toString(), coment);
    statusRequest = handlingData(response);
    if (StatusRequest.success == statusRequest) {
      if (response['status'] == "success") {
        getOrders();
